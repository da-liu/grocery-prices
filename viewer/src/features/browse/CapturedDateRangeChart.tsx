...
export function CapturedDateRangeChart({
  ...
  const activeAfter = draft?.capturedAfter ?? capturedAfter;
  const activeBefore = draft?.capturedBefore ?? capturedBefore;
  ...
  function onPointerDown(handle: "min" | "max") {
    return (e: React.PointerEvent) => {
      ...
      const initial = { capturedAfter, capturedBefore };
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    setDraft((current) => {
      const base = current ?? { capturedAfter, capturedBefore };
      const baseAfter = base.capturedAfter ?? min;
      const baseBefore = base.capturedBefore ?? max;
      let next: { capturedAfter: string | null; capturedBefore: string | null };
